...
function formatTrackedTime(seconds) {
    ...
    if (days > 0)
        timeParts.push(days > 1 ? `${days}д.` : '1д.')
    if (hours > 0)
        timeParts.push(`${hours}ч.`)
    if (minutes > 0)
        timeParts.push(`${minutes}мин.`)
...
export const TrackerStorage = {
    ...
    getTrackLength: (taskId) => {
        if (!TrackerStorage.isTracking(taskId))
            return [null, 0]
...
export function Tracker ({ taskId, trackedTime, setTrackedTime }) {
    ...
    const stopTracking = async () => {
        const [_, trackedSeconds] = TrackerStorage.getTrackLength(taskId)
    ...
    return (
        <div className='tracker-div'>
            ...
            <div className='tracker-info-total-time'>{formatTrackedTime(trackedTime)}</div>
            <ActionButton />
        </div>
    )
}
